...
*/
function readMetadata(reader: FileReader): pdfdom.PDF {
...
references.
*/
function findObject(file: File,
                    reference: pdfdom.IndirectReference,
...
export function open(filepath: string): void {
  ...
  var Info = findObject(file, <pdfdom.IndirectReference>pdf.trailer['Info'], pdf.cross_references);
  term.print('trailer->Info', Info);
  var Root = findObject(file, <pdfdom.IndirectReference>pdf.trailer['Root'], pdf.cross_references);
  term.print('trailer->Root', Root);
  var Pages = findObject(file, <pdfdom.IndirectReference>Root['Pages'], pdf.cross_references);
  ...
  for (var i = 0, page; (page = pages[i]); i++) {
    var page_object = findObject(file, <pdfdom.IndirectReference>page, pdf.cross_references);
  ...
  for (var i = 0, page_object: pdfdom.PDFObject; (page_object = page_objects[i]); i++) {
    ...
    for (var j = 0, page_content; (page_content = page_contents[j]); j++) {
      var content_object = findObject(file, <pdfdom.IndirectReference>page_content, pdf.cross_references);
